...
public class WLSC_169216_WNP_REG_CancelWLNToTelusPortin extends BaseTest {
	...
	@Test(groups = { "WLSC_169216_WNP_REG_CancelWLNToTelusPortin", "Port-In", "CompleteRegressionSuite", "Batch1" })
	public void testMethod_CancelWLNToTelusPortin(ITestContext iTestContext) throws Exception {
		...
		/*** STEPS FOR CREATING CONFIRMED RESPONSE FROM SMG ***/
		Reporting.setNewGroupName("SMG - CREATE PORT OUT RESPONSE WITH CONFIRMED STATUS");
		SMGICPPortSelectionPageSteps.createPortOutResponseWithConfirmFromSMGForWLN(smgTestData, foreignSub, foreignBan);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*
		 * CANCEL Request After sending confirm from SMG
		 */

		/*** STEPS FOR CANCEL RQUEST VALIDATION FROM PAC ***/
