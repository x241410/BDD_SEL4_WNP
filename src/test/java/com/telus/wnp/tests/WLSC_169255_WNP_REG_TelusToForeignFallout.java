...
/**
 ****************************************************************************
 * > DESCRIPTION: Telus to Foreign Port out _Fallout 8A > AUTHOR: x241410 > Test
 * Case: TC05_WNP_REG_Port Out_Telus to Foreign Port out _Fallout 8A _Skip
 * Validation
 ****************************************************************************
 */

public class WLSC_169255_WNP_REG_TelusToForeignFallout extends BaseTest {

	String sub = null;
	String ban = null;
	static String internalReqNumFromPAC = null;
	...
	@Test(groups = { "WLSC_169255_TelusToForeign8ASkipValidation", "Port-Out", "CompleteRegressionSuite" })
	public void testMethod_TelusToForeignPortout8A(ITestContext iTestContext) throws Exception {
		...
		/*** STEPS FOR VERIFYING 8A DELAY CODE FROM SMG ***/
		Reporting.setNewGroupName("SMG - REJECTION CODE 8A VALIDATION");
		SMGICPPortDetailsPageSteps.navigateToSMGAndVerifyRejectionCode(smgTestData, sub);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		// *** STEPS FOR SKIP VALIDATION FROM PAC **
		Reporting.setNewGroupName("PAC - SKIP VALIDATION");
		PACDashboardPageSteps.doSkipValidationsFromPAC(pacTestData, sub);
		PACDashboardPageSteps.verifyPortSummary(pacTestData, "intermediate2");
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
