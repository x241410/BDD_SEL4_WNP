...
/**
 ****************************************************************************
 * > DESCRIPTION: Intercarrier portout request when preport authentication is
 * enabled > AUTHOR: x241410 > Test Case: "TC01_WNP_REG_WWUP-429_Verfiy that no
 * 2FA is followed for Intercarrier portout request when preport authentication
 * is enabled_Telus
 ****************************************************************************
 */

public class WLSWNP_1186_No2FA_PreportEnabled extends BaseTest {
	...
	@Test(groups = { "WLSWNP_1186_No2FA_PreportEnabled", "Port-Out", "CompleteRegressionSuite" })
	public void testMethod_No2FAPreportEnabled(ITestContext iTestContext) throws Exception {
		...
		PACDashboardPageSteps.enablePreportAuthorization(pacTestData, sub, prePortDelayTimeInMinutes, "wireless");
		BaseSteps.Waits.waitGeneric(prePortDelayTimeInMinutes * 60);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
		...
		Reporting.setNewGroupName("SMART DESKTOP - SUB PREVALIDATION");
		SmartDesktopSearchPageSteps.verifySubscriberStatusFromSD(sdTestData, sub, ban, "active");
		SmartDesktopDashboardPageSteps.verifyNo2FASentFromCommnicationHistory(
				sdTestData.getString("NO_SEARCH_RESULTS_FOUND_COMM_HISTORY_MSG"), sub);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
		...
		SMGICPPortDetailsPageSteps.verifyResponseFromSMGICPPortDetails(smgTestData, sub,
		...
		SmartDesktopDashboardPageSteps.navigateToCommunicationHistoryTabToVerifyEmail(sdTestData, sub);
		SmartDesktopDashboardPageSteps.openPreviewAndCaptureEmail(sdTestData.getString("EXPECTED_SUBJECT_TITLE"));
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
