...
public class WLSC_169241_WNP_REG_UndoCancelFromSOA extends BaseTest {
	...
	@Test(groups = { "WLSC_169241_WNP_REG_PerformUndoCancelFromSOA", "Port-In", "CompleteRegressionSuite" })
	public void testMethod_PerformUndoCancelFromSOA(ITestContext iTestContext) throws Exception {
		...
		Reporting.setNewGroupName("SMG - CREATE PORT OUT RESPONSE");
		// formattedSub = GenericUtils.getHyphenSeparatedTN(foreignSub);
		SMGICPPortSelectionPageSteps.createPortOutResponseWithConfirmFromSMG(smgTestData, foreignSub, sub);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
		...
		SMGSOASelectionPageSteps.navigateToSMGSOASelectionPage();
		SMGSOASelectionPageSteps.queryFromSOASelectionPage("fromtn", foreignSub);
		SMGSOASelectionPageSteps.performUndoCancelFromSOA(soaTestData);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
