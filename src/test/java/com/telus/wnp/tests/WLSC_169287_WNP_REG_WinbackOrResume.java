...
/**
 ****************************************************************************
 * > DESCRIPTION: Winback or Resume > AUTHOR: x241410 > Test Case:
 * TC21_WNP_REG_Winback or Resume
 ****************************************************************************
 */
public class WLSC_169287_WNP_REG_WinbackOrResume extends BaseTest {
	...
	@Test(groups = { "WLSC_169287_WinbackOrResume", "Port-In", "Port-Out", "Winback", "CompleteRegressionSuite" })
	public void testMethod_WinbackOrResume(ITestContext iTestContext) throws Exception {
		...
		/*** STEPS FOR PERFORMING RESUME FROM SMART DESKTOP ***/
		Reporting.setNewGroupName("SMART DESKTOP - RESUME REQUEST");
		SmartDesktopDashboardPageSteps.submitResumeRequestFromSD(sdTestData, sub, sdTestData.getString("IMEI"));
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM CODA ***/
		Reporting.setNewGroupName("CODA - POST RESUME");
		CODASearchPageSteps.verifySubscriberStatusFromCODA(codaTestData, sub, "active");
		String expectedBrand = codaTestData.getString("EXPECTED_BRAND_NAME");
		CODASearchPageSteps.verifyCurrentBrandFromCODA(expectedBrand);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM RCM ***/
		Reporting.setNewGroupName("RCM - POST RESUME");
		RCMPageSteps.verifySubscriberStatusFromRCM(rcmTestData, sub, "active");
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM SMARTDESKTOP ***/
		Reporting.setNewGroupName("SMART DESKTOP - POST RESUME");
		SmartDesktopSearchPageSteps.verifySubscriberStatusFromSD(sdTestData, sub, ban, "active");
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
