...
/**
 ****************************************************************************
 * > DESCRIPTION: Send Cancellation to External Service Provider Future Date >
 * AUTHOR: x241410 > Test Case: TC07_WNP_REG_Send Cancellation to External
 * Service Provider _confirm flow _Future Date
 ****************************************************************************
 */

public class WLSC_169271_WNP_REG_SendCancelESPFDate extends BaseTest {
	...
	@Test(groups = { "WLSC_169271_WNP_REG_SendCancelESPFDate", "SendCancelation", "CompleteRegressionSuite" })
	public void testMethod_SendCancelESPFDate(ITestContext iTestContext) throws Exception {
		...
		SMGICPPortDetailsPageSteps.verifyResponseFromSMGICPPortDetails(smgTestData, foreignSub, expectedResponseType);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		// Need to be executed on the next day
		 ...
		 * //int DelayTime = smgTestData.getInt("DDDT_DELAY_IN_MINUTES") + 1; //int
		 * delayTimeInSeconds = DelayTime * 60 * 1000;
		 * //BaseSteps.Waits.waitGeneric(delayTimeInSeconds);
		 * //Reporting.logReporter(Status.INFO, "Wait for defined DDDT of " + DelayTime
		 * + " minutes."); //Reporting.printAndClearLogGroupStatements();
		 * 
		 *//*** STEPS FOR PORT STATUS VALIDATION FROM PAC ***/
		/*
		 * Reporting.setNewGroupName("PAC - PORT STATUS AND SUMMARY VALIDATION");
		 * PACDashboardPageSteps.VerifyPortSummaryAndPortStatus(pacTestData, foreignSub,
		 * "postValidation"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 * // *** STEPS FOR VALIDATE REQUEST STATUS FROM SMG **
		 * Reporting.setNewGroupName("SMG - VERIFY REQUEST STATUS");
		 * SMGICPPortSelectionPageSteps.verifyRequestStatusFromICPPortSelectionPage(
		 * foreignSub, "Port Out", smgTestData.getString("EXTERNAL_TU02_MOBILITY"),
		 * smgTestData.getString("SMG_STATUS_CONFIRMED"));
		 * WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 *//*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM SMARTDESKTOP ***/
		 ...
		 * ban, "cancelled"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 *//*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM CODA ***/
		/*
		 ...
		 * "cancelled"); WebDriverSteps.closeTheBrowser();
		 * Reporting.printAndClearLogGroupStatements();
		 * 
		 *//*** STEPS FOR SUBSCRIBER ACTIVE STATUS FROM RCM ***//*
																 * Reporting.setNewGroupName("RCM - SUB PREVALIDATION");
																 * RCMPageSteps.verifySubscriberStatusFromRCM(
																 * rcmTestData, sub, "cancelled");
																 * WebDriverSteps.closeTheBrowser();
																 * Reporting.printAndClearLogGroupStatements();
																 */
