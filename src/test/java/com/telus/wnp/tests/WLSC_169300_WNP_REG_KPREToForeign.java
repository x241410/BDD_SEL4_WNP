...
/**
 ****************************************************************************
 * > DESCRIPTION: Port Out_Koodo to Foreign Port out > AUTHOR: x241410 > Test
 * Case: TC03_WNP_REG_Port Out_Koodo to Foreign Port out _confirm flow
 ****************************************************************************
 */

public class WLSC_169300_WNP_REG_KPREToForeign extends BaseTest {
	...
	@Test(groups = { "WLSC_169300_WNP_REG_KPREToForeign", "Port-Out", "CompleteRegressionSuite" })
	public void testMethod_KPREToForeignPortOut(ITestContext iTestContext) throws Exception {
		...
		/**
		 * 
		 * pt 148 test data koodoPrepaidNum = "7780316518"; koodoPrepaidBan =
		 * "10000001922008";
		 * 
		 */
		...
		// Need to send response as Yes then RR entry will come in pac
		...
		/*** STEPS FOR VERIFYING 8A CODE FROM PAC ***/
		Reporting.setNewGroupName("PAC - 8A VALIDATION");
		PACDashboardPageSteps.VerifyPortSummaryAndPortStatus(pacTestData, koodoPrepaidNum, "intermediate1");
		String expectedCode = "8A";
		PACDashboardPageSteps.verifyResponseCodeFromPAC(expectedCode);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*** STEPS FOR MODIFY ACCOUNT DETAILS FROM SMG ***/
		Reporting.setNewGroupName("SMG - MODIFY REQUEST DETAILS");
		SMGICPPortDetailsPageSteps.navigateToSMGICPPortDetailsPage();
		SMGICPPortDetailsPageSteps.queryFromICPPortDetailsPage(smgTestData, koodoPrepaidNum);
		SMGICPPortDetailsPageSteps.performSUP3FromSMGICPPortDetailsPage(smgTestData, "BAN", koodoPrepaidBan);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
