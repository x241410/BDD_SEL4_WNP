...
public class WLSC_169248_WNP_REG_PortInIMEIModify extends BaseTest {
	...
	@Test(groups = { "WLSC_169248_WNP_REG_PortInIMEIModify", "Port-In", "CompleteRegressionSuite" })
	public void testMethod_PortInIMEIModify(ITestContext iTestContext) throws Exception {
		...
		// * // * Send RR with code 8H then Modify request from PAC Send Confirm from
		// SMG // *
		...
		int delay1 = smgTestData.getInt("DDDT_DELAY_IN_MINUTES") + 1;
		String ddt1 = GenericUtils.tempDDTMethod(delay1);
		PACDashboardPageSteps.modifyIMEIRequestFromPAC(pacTestData, imei, ddt1);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
		...
		/*** STEPS FOR VERIFYING THE SUBSCRIBER SPID AS TU02 ***/
		Reporting.setNewGroupName("LSMS API - SPID POST VALIDATION");
		Reporting.logReporter(Status.INFO, "STEP === SPID Validation --> SPID validation check via API call===");
		...
		String expectedSPIDTelus = lsmsTestData.getString("TELUS_SPID");
		String response = XmlUtils.toXml(apiOperation1.get("spidAPIfeatureCallResponse"));
		String actualSPIDTelus = GenericUtils.returnKeyValueFromXMLNode(response, "lspId");
		Validate.assertEquals(actualSPIDTelus, expectedSPIDTelus, "SPID is not as expected", true);
		Reporting.printAndClearLogGroupStatements();
