...
/**
 ****************************************************************************
 * > DESCRIPTION: _WLN to Koodo Port Modify DDT from PAC _SUP2 > AUTHOR: x241410
 * > Test Case: TC08_WNP_REG_WLN Port In_SUP_WLN to Koodo Port in with IMEI
 * Fallout _Modify DDT from PAC _SUP2
 ****************************************************************************
 */

public class WLSC_169219_WNP_REG_WLNKoodoPortInSUP2 extends BaseTest {
	...
	@Test(groups = { "WLSC_169219_WNP_REG_WLNKoodoPortInSUP2", "Port-In", "CompleteRegressionSuite", "Batch1" })
	public void testMethod_WLNKoodoPortInSUP2(ITestContext iTestContext) throws Exception {
		...
		int delayInMinutes = pacTestData.getInt("WLN_DDT_DELAY_IN_MINUTES") + 2;
		...
		PACDashboardPageSteps.modifyDDDTFromPAC_WLN(pacTestData, ddt);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();

		/*** STEPS FOR CREATE PORT OUT RESPONSE FROM SMG ***/
		Reporting.setNewGroupName("SMG - CREATE PORT OUT RESPONSE");
		SMGICPPortSelectionPageSteps.createPortOutResponseWithConfirmFromSMGForWLN(smgTestData, foreignSub, sub);
		WebDriverSteps.closeTheBrowser();
		Reporting.printAndClearLogGroupStatements();
...
/*	
	...
	*//***
		 * STEPS TO MODIFY DDT FROM SMG- CHECK CHECKBOX FROM SELECTION PAGE CLICK TU
		 * REQUEST NUMBER THEN MODIFY FROM DETAILS PAGE
		 ***//*
				 * Reporting.setNewGroupName("SMG - MODIFY DDT FROM SMG"); SMGICPPortDetailsPage
				 * SMGICPPortDetailsPage = new SMGICPPortDetailsPage();
				 * SMGICPPortSelectionPageSteps.navigateToSMGICPPortSelectionPage();
				 * 
				 * String portType = smgTestData.getString("PORT_TYPE_SELECT_PORT_OUT"); String
				 * newNetwork = smgTestData.getString("TELUS_TU04_MOBILITY");
				 * 
				 * SMGICPPortSelectionPage SMGICPPortSelectionPage = new
				 * SMGICPPortSelectionPage();
				 * SMGICPPortSelectionPageSteps.queryFromICPPortSelectionPageFromOSP(sub,
				 * portType, newNetwork);
				 * BaseSteps.Waits.waitForElementVisibilityLongWait(SMGICPPortSelectionPage.
				 * firstRowCheckBox);
				 * BaseSteps.Clicks.clickElement(SMGICPPortSelectionPage.firstRowCheckBox);
				 * BaseSteps.Clicks.clickElement(SMGICPPortSelectionPage.
				 * clickFirstRequestNumFromSearchResult);
				 * 
				 * BaseSteps.Windows.switchToNewWindow(); if
				 * (SMGICPPortSelectionPage.getcanSVCRequestConfirmationPopUpBtnStatus()) {
				 * BaseSteps.Clicks.clickElement(SMGICPPortSelectionPage.
				 * cancelSVCRequestConfirmationPopUpBtn); }
				 * 
				 * BaseSteps.Windows.switchToNewWindow();
				 * 
				 * int intNewDelay = smgTestData.getInt("DDDT_DELAY_IN_MINUTES"); String
				 * updatedDDDT = GenericUtils.getDDDTWithDelay(intNewDelay);
				 * Reporting.logReporter(Status.INFO, "Updated DDDT is: " + updatedDDDT);
				 * BaseSteps.Waits.waitForElementVisibility(SMGICPPortDetailsPage.
				 * changeDDDTField, 60);
				 * BaseSteps.SendKeys.clearFieldAndSendKeys(SMGICPPortDetailsPage.
				 * changeDDDTField, updatedDDDT);
				 * 
				 * SMGICPPortSelectionPageSteps.saveRequest();
				 * 
				 * BaseSteps.Waits.waitForElementVisibilityLongWait(SMGICPPortDetailsPage.
				 * requestSubmissionMsg); String expectedText =
				 * SMGICPPortDetailsPage.getRequestSubmissionMsg(); String actualText =
				 * testDataJson.getString("REQUEST_SUBMISSION_MESSAGE");
				 * 
				 * Validate.assertEquals(actualText, expectedText,
